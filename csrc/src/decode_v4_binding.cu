...
torch::Tensor turboquant_decode_v4(
    ...
    if (const char* dbg = std::getenv("TQ_DEBUG_STRIDES")) {
        (void)dbg;
        std::printf("[decode_v4]  ebs=%d dc=%u | q_stride (h,n,page)=(%u,%u,%u) | n_stride (h,n,page)=(%u,%u,%u) k_base=%p v_base=%p k_norms=%p v_norms=%p\n",
...
torch::Tensor turboquant_decode_v4_from_cache(
    ...
    if (const char* dbg = std::getenv("TQ_DEBUG_STRIDES")) {
        (void)dbg;
        std::printf("[from_cache] ebs=%d dc=%u qbytes=%d nbytes=%d | q_stride (h,n,page)=(%u,%u,%u) | n_stride (h,n,page)=(%u,%u,%u) k_base=%p v_base=%p k_norms=%p v_norms=%p\n",
