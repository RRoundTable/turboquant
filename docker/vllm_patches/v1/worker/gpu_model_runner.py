        ...
        for group in self._kv_cache_spec_attn_group_iterator():
            ...
            for layer_name in group.layer_names:
                ...
                if isinstance(kv_cache_spec, AttentionSpec):
                    ...
                    # Use the backend's dtype hint if the shape doesn't match
                    # the standard element count (e.g., quantized backends
                    # that pack data more tightly than head_size elements).
                    ...
                    for s in kv_cache_shape:
                        shape_elements *= s
                    # Allow backends to use a subset of the allocated
                    # buffer (e.g., quantized KV with smaller element size).
    ...
    def get_kv_cache_spec(self) -> dict[str, KVCacheSpec]:
        ...
        for layer_name, attn_module in attn_layers.items():
            ...
            if spec := attn_module.get_kv_cache_spec(self.vllm_config):
                # Allow backends to override page size for compressed KV.
                if hasattr(attn_module, 'attn_backend'):
                    backend = attn_module.attn_backend
                    if hasattr(backend, 'get_kv_cache_page_size'):
